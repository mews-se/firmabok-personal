...
import { InvoicePDF, localizeVatNotice } from '@/lib/invoices/pdf-template'
...
function renderText(invoice: Invoice, language: 'sv' | 'en'): string {
...
const exportInvoice = (overrides: Partial<Invoice> = {}): Invoice =>
  makeInvoice({
    id: 'inv-1',
    status: 'sent',
...
describe('invoice PDF statutory notices follow the document language', () => {
  ...
  it('stamps the shared constants at create time so the PDF match cannot drift', () => {
    ...
    expect(getVatRules('eu_business', true).reverseChargeText).toBe(EU_REVERSE_CHARGE_NOTICE)
  })
})
