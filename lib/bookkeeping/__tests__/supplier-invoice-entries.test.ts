...
describe('a supplier invoice that nets below zero books its anchor on the debit side', () => {
  ...
  it('privately paid: owner account D 300', async () => {
    const invoice = makeSupplierInvoice({ subtotal: -300, vat_amount: 0, total: -300 })
    await createSupplierInvoicePrivatelyPaidEntry(null as never, 'company-1', 'user-1', invoice, items, 'aktiebolag')
    const input = mockedCreateEntry.mock.calls[0][3]
    expect(findByAccount(input.lines, '2893')[0]).toMatchObject({ debit_amount: 300, credit_amount: 0 })
    assertBalanced(input)
  })
})
